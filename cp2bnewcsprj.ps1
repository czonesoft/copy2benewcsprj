# C#项目复制自动化脚本
# 陈震军 2024/12/23
$oldName = "LZOH.AudCLient"
$newName = "LZOH.CPECLient"
...
Get-ChildItem -Path $newName -Filter "*.csproj.user" -Recurse | ForEach-Object {
    ...
    Set-Content -Path (Join-Path $_.DirectoryName $newUserFileName) -Value $content -Encoding UTF8
...
Get-ChildItem -Path $newName -Filter "*.sln" -Recurse | ForEach-Object {
    ...
    Set-Content -Path (Join-Path $_.DirectoryName $newFileName) -Value $content -Encoding UTF8
    Remove-Item $_.FullName
}
